...
/* atendimento ao cidadao */
    ...
    void escreverAtendimentosNoArquivo(char arquivo[20], char servico_solicitado, atendimento atendimentos_realizados[], int tam_at_rlzd) {
        int i;
        FILE *arq;
        arq = fopen(arquivo, "w");
        if(arq == NULL) {
            ...
            for(i=0;i<tam_at_rlzd;i++) {
                if(atendimentos_realizados[i].servico_solicitado == servico_solicitado) {
